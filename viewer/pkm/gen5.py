...
def parse_gen5(raw: bytes) -> dict:
    out = default_out("gen5")
    ...
    out["nickname"] = read_utf16(raw, 0x48, 11)
    out["original_trainer"] = read_utf16(raw, 0x68, 8)
    out["met_level"] = raw[0x84] & 0x7F
    out["ball_id"] = raw[0x83]
    out["is_egg"] = bool((iv32 >> 30) & 1)
    out["level"] = raw[0x8C] if len(raw) > 0x8C and raw[0x8C] else out["met_level"]
    out["ivs"] = {
        "hp": iv_field(iv32, 0),
        "atk": iv_field(iv32, 5),
